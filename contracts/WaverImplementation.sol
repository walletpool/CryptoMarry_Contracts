...
/* Enum Statuses of the Marriage*/
    ...
    mapping (address => mapping(uint => uint8)) public wasDistributed;  //Tracking whether NFT has been distributed between partners upon divorce
...
/**
    ...
    function createProposal(
        ...
        if (_votetype == Type.Divorce) {
            ...
            require(props.proposed == msgSender_ || props.proposer == msgSender_);
        }
...
/**
    ...
    function executeVoting (uint256 _id, 
                           uint256 txfee, 
        ...
        onlyAccess
    {
        ...
        //Staking ETH to the Compound protocol. cETH token is recieved. 
            ...
            processtxn(addressWaveContract, _cmfees);
                ...
                TransferHelper.safeApprove(voteProposal.tokenID, voteProposal.receiver, _amount);
...
/* Managing family members*/
  ...
  function checkPartner (address _member) internal view returns (address msgSender__){
        address msgSender_ = _msgSender();
 ...
 /**
    ...
    function withdrawERC20(address _tokenID) external onlyAccess{
        ...
        amount = IERC20Upgradeable(_tokenID).balanceOf(address(this));
            ...
            require(transferToken(_tokenID, addressWaveContract, amount * props.cmFee/ 10000))
        ;
        amount = IERC20Upgradeable(_tokenID).balanceOf(address(this));
...
/**
  ...
  function SplitNFT (address _tokenAddr, uint _tokenID, string memory nft_json1, string memory nft_json2 ) external onlyAccess{
        ...
        require (checkOwnership(_tokenAddr,_tokenID) == true); // Check whether the indicated token is owned by the proxy contract.     
         ...
         nftSplit.splitNFT(_tokenAddr, _tokenID, nft_json1, nft_json2, props.proposer, props.proposed, address(this)); //A copy of the NFT is created by NFT Splitter.  
...
/**
    ...
    function checkOwnership (address _tokenAddr, uint256 _tokenID) internal  view returns (bool){
            address _owner; 
...
/**
    ...
    function sendNft (address _tokenAddr, address _receipent, uint _tokenID  ) external {
     WaverContract _wavercContract = WaverContract(addressWaveContract);
    require (_wavercContract.addressNFTSplit() == msg.sender);
     IERC721(_tokenAddr).safeTransferFrom(address(this),_receipent, _tokenID);
}
