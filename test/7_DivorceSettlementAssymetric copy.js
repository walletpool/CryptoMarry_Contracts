...
describe("Divorce Settlement Functions Assymetric - Proposer - 20%, Proposed - 80%", function () {
  ...
  it("Divorce will divide ETH balance according to share (20/80)", async function () {
    ...
    await expect(
      ...
      [
        ...
        hre.ethers.utils.parseEther("1.9602"),
        hre.ethers.utils.parseEther("7.8408"),
      ]
    );
  });

  it("After divorce ERC20 tokens can be split according to share (20/80)", async function () {
    const { instance, accounts, WavePortal7 } = await loadFixture(
      deployTokenFixture
    );
    ...
    await expect(() =>
      ...
      [
        hre.ethers.utils.parseEther("-50"),
        hre.ethers.utils.parseEther("9.9"),
        hre.ethers.utils.parseEther("39.6"),
  ...
  it("After divorce NFT tokens can be split between partners according to share (2/8)", async function () {
    ...
    await expect(await nftSplit.balanceOf(accounts[0].address, id)).to.equal(2);
    await expect(await nftSplit.balanceOf(accounts[1].address, id)).to.equal(8);
  });
  ...
  it("Partner can acquire all copies of split NFT and receive it", async function () {
    ...
    txn = await nftSplit
      .connect(accounts[0])
      .safeTransferFrom(accounts[0].address, accounts[1].address, id, 2, 0x0);
