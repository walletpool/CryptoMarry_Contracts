...
async function main() {
  ...
  console.log("Proposal has been sent");
  ...
  txn = await WavePortal7.execute({ value: hre.ethers.utils.parseEther("11") });
  console.log("*************************************")
  console.log("Balance of ETH :",await hre.ethers.provider.getBalance(accounts[0].address));

  console.log("Proposal has been executed *****************", txn.gasLimit);
  ...
  txn = await instance.claimToken(1);
  expect(await WavePortal7.balanceOf(accounts[0].address)).to.equal(
    hre.ethers.utils.parseEther("199.5")
  );

  // txn = await instance.connect(accounts[3]).claimToken(1);
 ...
 console.log("Balance of ETH :",await hre.ethers.provider.getBalance(instance.address));
  ...
  expect(await WavePortal7.balanceOf(accounts[0].address)).to.equal(
    ...
    txn = await instance.connect(accounts[1]).claimToken(1);
...
// Sending ERC721 Token 
    txn = await instance.createProposal("test4", 5,1,0, hre.ethers.utils.parseEther("1"),accounts[2].address,nftContract.address,1,1);
      ...
      txn = await instance.connect(accounts[2]).claimToken(1);
      ...
      txn = await instance.connect(accounts[4]).claimToken(1);
      ...
      txn = await instance.connect(accounts[5]).claimToken(1);
      ...
      txn = await instance.connect(accounts[6]).claimToken(1);
