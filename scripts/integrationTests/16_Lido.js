...
wethAddress, txn, instance, LidoFacet;
...
describe("Lido Integration Test", function () {
    before(async () => {
    ...
    it('Users can connect Lido App', async () => {
        const cut = []
    ...
    it('Third party Users cannot connect Lido App', async () => {
        const cut = []
        const diamondCut = await ethers.getContractAt('IDiamondCut', instance.address);
        cut.push({
            facetAddress: LidoFacet.address,
    ...
    it('Users can disconnect Lido App', async () => {
        const cut = []
        const diamondCut = await ethers.getContractAt('IDiamondCut', instance.address);
        let tx;
        let receipt;
        ...
        txn = await instance.getAllConnectedApps();
        expect (txn).contain(LidoFacet.address);
        const instanceLido = await ethers.getContractAt('LidoFacet', instance.address);
        let remove = []; 
        remove.push({
    ...
    it('Users can deposit assets on Lido', async () => {
        ...
        const steth = new ethers.Contract(stethAddress, wethAbi, signer);
        ...
        txn = await instanceLido.executeSubmitLido(1); // voteID
        txn = await instance.getVotingStatuses(1);
        expect(txn[0].voteStatus).to.equal(899);

        txn = await steth.callStatic.balanceOf(instance.address);

        await advanceBlockHeight(10000);
        txn = await steth.callStatic.balanceOf(instance.address);
